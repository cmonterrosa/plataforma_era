class NivelesController < ApplicationController
  ...
  def index
    ...
    @sostenimiento = current_user.sostenimiento
    @usuarios = User.find(:all,
                                      :joins => "users, escuelas e",
                                      :conditions => ["users.escuela_id = e.id AND e.nivel_id = ? AND e.sostenimiento = ?", @nivel.id, @sostenimiento]).paginate(:page => params[:page], :per_page => 25) if @nivel && @sostenimiento
    @load = 1
    ...
    def showResults
    @nivel = Nivel.find(current_user.nivel_id) if current_user.nivel_id
    @sostenimiento = User.find(current_user).sostenimiento if current_user
    @users = User.find_by_sql("SELECT usr.* FROM users as usr
                               LEFT JOIN escuelas as esc on esc.id = usr.escuela_id
                               WHERE esc.nivel_id = #{@nivel.id} AND esc.sostenimiento = '#{@sostenimiento}' AND esc.clave like '%#{params[:clave_nombre].to_s}%'
                               OR usr.login like '%#{params[:clave_nombre].to_s}%'") if (params[:buscar] == "nombre")
    @users = User.find_by_sql("SELECT usr.* FROM users as usr
                               LEFT JOIN escuelas as esc on esc.id = usr.escuela_id
                               WHERE esc.nivel_id = #{@nivel.id} AND esc.sostenimiento = '#{@sostenimiento}'  AND esc.nombre like '%#{params[:clave_nombre].to_s}%'
                               OR usr.nombre like '%#{params[:clave_nombre].to_s}%'") if (params[:buscar] == "clave")
    @users = User.find_by_sql("SELECT usr.* FROM users as usr
                               LEFT JOIN escuelas as esc on usr.escuela_id = esc.id
                               WHERE esc.nivel_id = #{@nivel.id} AND esc.sostenimiento = '#{@sostenimiento}'  AND esc.estatu_id = #{params[:estatus_escuela].to_i}") if (params[:buscar] == "estatus")
